...
export default function AdminPanel({ partners }: { partners: Partner[] }) {
  ...
  const grantAccess = async () => {
    ...
    if (!user) { toast.error("משתמש לא נמצא — עליו להתחבר תחילה"); return; }
    if (!grantPartnerId) { toast.error("בחר תיק עבודות"); return; }
    setBusy(true);
    ...
    toast.success(`הוענקה גישה ל-${user.email} לתיק ${partnerOf(grantPartnerId)}`);
    setGrantEmail("");
    load();
  };
  ...
  const makeAdmin = async () => {
    ...
    if (error) { toast.error(error.message); return; }
    toast.success(`${user.email} הפך למנהל`);
    setAdminEmail("");
    load();
  };
  ...
  return (
    <div className="space-y-6">
      <AlertDialog open={!!revokeTarget} onOpenChange={() => setRevokeTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>ביטול גישה?</AlertDialogTitle>
            <AlertDialogDescription>משתמש זה יאבד מיידית את הגישה לתיק זה.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={!!removeAdminTarget} onOpenChange={() => setRemoveAdminTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{emailOf(removeAdminTarget ?? "")} יאבד את כל הרשאות המנהל.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
          <span className="font-semibold text-sm">🔑 גישה לתיקים</span>
          <div className="flex items-center gap-3">
            {loading && <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />}
            <span className="text-xs text-gray-500">{access.length} שורות גישה</span>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 text-xs text-gray-400">
                ...
                <th className="px-4 py-2.5 text-right font-medium">תיק עבודות</th>
                <th className="px-4 py-2.5 text-right font-medium">הוענקה</th>
              ...
              {access.map((row) => (
                <tr key={row.id} className="border-b border-gray-800/40 hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-2.5">
                    <button onClick={() => setRevokeTarget(row.id)} className="text-red-500 hover:text-red-400 text-xs font-medium">בטל גישה</button>
                  </td>
                </tr>
              ))}
              {access.length === 0 && (
                <tr><td colSpan={4} className="text-center py-6 text-gray-600 text-sm">אין שורות גישה עדיין</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        <div className="text-sm font-semibold mb-3">➕ הענק גישה</div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={grantAccess} disabled={busy || !grantEmail || !grantPartnerId} size="sm">הענק</Button>
        </div>
        <p className="text-xs text-gray-600 mt-2">על המשתמש להתחבר לפחות פעם אחת כדי להופיע בהשלמה אוטומטית.</p>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        <div className="space-y-2 mb-4">
          {admins.map((a) => (
            <div key={a.user_id} className="flex items-center justify-between bg-gray-800/60 rounded-lg px-3 py-2">
              ...
              <button onClick={() => setRemoveAdminTarget(a.user_id)} className="text-red-500 hover:text-red-400 text-xs">הסר</button>
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          <Input
            ...
            list="cp-users-list"
          />
          <Button variant="outline" onClick={makeAdmin} disabled={!adminEmail} size="sm">הפוך למנהל</Button>
        </div>
      </div>

      {/* Known users */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-800 font-semibold text-sm">👤 משתמשים ידועים ({users.length})</div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 text-xs text-gray-400">
                <th className="px-4 py-2 text-right font-medium">Email</th>
                <th className="px-4 py-2 text-right font-medium">נראה לאחרונה</th>
                <th className="px-4 py-2 text-right font-medium">תיקים</th>
              </tr>
            </thead>
            <tbody>
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} className="border-b border-gray-800/40 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-2.5 text-xs">
                      {userAccess.length === 0
                        ? <span className="text-gray-600">אין גישה</span>
