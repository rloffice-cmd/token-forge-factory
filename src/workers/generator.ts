 ...
 * 
 * IMPORTANT: Use raw string for regex pattern to avoid escaping issues
 */
 ...
 * 
 * Note: We build the code as an array and join to avoid
 * template literal escaping issues with backslashes
 */
export function generateSolution(task: Task, fixNotes?: string[]): GeneratorOutput {
  try {
    ...
    // Build code line by line to preserve backslashes correctly
    const lines = [
      '"""',
      ...
      '            continue',
      '            ',
      `        # Validate year range: ${minYear} to ${maxYear}`,
      '        year_int = int(year)',
